...
16. `restart` reads `PREVIA_HOME/run/up-state.json`, stops the detached local
    processes, starts a new detached stack with the same runner topology, and
    rewrites the runtime file with new PIDs.
17. `restart` fails clearly when no detached runtime file exists.
18. `up --detach` fails clearly when `PREVIA_HOME/run/up-state.json` already
    exists.
19. `uninstall` without `--purge` removes binaries and runtime state but
    preserves `PREVIA_HOME/config` and `PREVIA_HOME/data`.
20. `uninstall --purge` removes the entire `PREVIA_HOME` tree.
21. Any file generated by `previactl` is written under `PREVIA_HOME`.
