...
pub fn project_spec_from_row(row: &sqlx::any::AnyRow) -> ProjectSpecRecord {
    let spec_json = row
    ...
    let urls_json = row
        ...
        .unwrap_or_else(|_| "[]".to_owned());
    let sync_value = boolish_column(row, "sync");
    let live_value = boolish_column(row, "live");
...
fn boolish_column(row: &sqlx::any::AnyRow, column: &str) -> bool {
...
pub async fn delete_project_spec_record(
    ...
    let mut tx = db.begin().await?;
    let result = db
        .query("DELETE FROM project_openapi_specs WHERE project_id = ? AND id = ?")
        .bind(project_id)
        .bind(spec_id)
        .execute(&mut *tx)
        .await?;
...
pub async fn backfill_project_spec_md5_hashes(db: &DbPool) -> Result<u64, sqlx::Error> {
    let rows = db
        .query(
            "SELECT id, spec_json
        FROM project_openapi_specs
